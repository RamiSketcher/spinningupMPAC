...
def mlp_actor_critic(x, a,
    ...
    with tf.variable_scope('v'):
        v = value_function_mlp(x) # If V'(st+1) for eq#8
...
"""
def reward_dynamic_model(x, a, pi, hidden_sizes=(256,256), activation=tf.nn.relu, action_space=None):

    value_function_mlp = lambda x : tf.squeeze(mlp(x, list(hidden_sizes)+[1], activation, None), axis=1)
    
    with tf.variable_scope('dm'):
        transition= dynamic_model(x,a) # st+1 = f(st,at)
    with tf.variable_scope('dm',reuse=True):
        transition_pi = dynamic_model(x,pi) # st+1_pi = f_pi(st,pi(at|st))

    with tf.variable_scope('rm'):
        r_rm = reward_model(x,a) # r(st,at)
    with tf.variable_scope('rm',reuse=True):
        r_rm_pi = reward_model(x,pi) # r_pi(st,pi(at|st))
...
def dynamic_model(x, a, hidden_sizes=(256,128), activation=tf.nn.relu, output_activation=None):
    state_dim = x.shape.as_list()[-1] # Count the # of elements in tensor x
    x = tf.concat([x,a],-1) # State-Action input (st,at), concat(arrays, axis)

    for h in hidden_sizes:
        x = tf.layers.dense(x, units=h, activation=activation)
    # at ~ pi(at|st) = N(mu,std)
    ...
    transition = mu + tf.random_normal(tf.shape(mu)) * std # st+1 = N(mu,std)
    # Why not learning delta = st+1 - st ?
    return transition


def reward_model(x, a, hidden_sizes=(256,128), activation=tf.nn.relu, output_activation=None):
    x = tf.concat([x,a],-1)
