...
"""
# InvertedPen
def memb_pe(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), Model=core.MLPModel,
        ...
        batch_size=100, start_steps=4000,
        max_ep_len=1000, save_freq=1,# max_ep_len for locomotion = 1k, for DClaw = 40
        train_model_epoch=1, test_freq=2, save_epoch=100,
        exp_name='', env_name='',
        logger_kwargs=dict()):
    ...
    for t in range(total_steps):
        ...
        # elif t % 500 == 0: # Only update model (every 500 t)
            ...
            print(colorize('Model training before learning (Initial)..', color='yellow', bold=True))
            model.train_model(replay_buffer, t+1, batch_size, 5)
